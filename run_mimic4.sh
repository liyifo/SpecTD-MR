...
PYTHONHASHSEED=1203 python -B -m pretrain.train_pretrain \
  ...
  --ddi-path data/MIMIC-IV/ddi_A_final.pkl \
  --batch-size 64 \
  --lr 0.001 \
  --feature-dim 64 \
  --hidden-dim 128 \
  ...
  --warmup2-epochs 80 \
  --warmup2-patience 10 \
  --warmup2-best-path pretrain_logs/mimic4/mimic4-run-01/warmup2_best.pt \
  --epochs 120 \
  --full-patience 15 \
  --full-best-path pretrain_logs/mimic4/mimic4-run-01/stage1_full_best.pt \
  --log-dir pretrain_logs/mimic4/mimic4-run-01/stage1 \
  --save-checkpoint \
  --device cuda:0 \
  --python-seed 1203 \
  --numpy-seed 2048 \
...
PYTHONHASHSEED=1203 python -B -m recommender.train_downstream \
  ...
  --pretrain-checkpoint pretrain_logs/mimic4/mimic4-run-01/stage1/nc5_h4_fd64_clw0.75_alw0.05/stage1_final.pt \
  ...
  --bootstrap-rounds 10 \
  --log-dir downstream_logs/mimic4/mimic4-run-01 \
  --device cuda:0 \
  --python-seed 1203 \
  --numpy-seed 2048 \
  --torch-seed 1203
