...
def _build_concept_cooccurrence(records, diag_size, proc_size, med_size, output_path='concept_cooccurrence.pkl'):
    total_size = diag_size + proc_size + med_size
    proc_offset = diag_size
    med_offset = diag_size + proc_size
    co_matrix = np.zeros((total_size, total_size), dtype=np.float32)
    occurrence = np.zeros(total_size, dtype=np.float32)

    for patient in records:
        for visit in patient:
            diag_nodes = {code for code in visit[0] if 0 <= code < diag_size}
            proc_nodes = {code + proc_offset for code in visit[1] if 0 <= code < proc_size}
            med_nodes = {code + med_offset for code in visit[2] if 0 <= code < med_size}
            visit_nodes = list(diag_nodes | proc_nodes | med_nodes)
            if not visit_nodes:
                continue
            for node_idx in visit_nodes:
                occurrence[node_idx] += 1
            for i, src in enumerate(visit_nodes):
                co_matrix[src, src] += 1
                for dst in visit_nodes[i + 1:]:
                    co_matrix[src, dst] += 1
                    co_matrix[dst, src] += 1

    for idx in range(total_size):
        if occurrence[idx] > 0:
            co_matrix[idx] /= occurrence[idx]
        co_matrix[idx, idx] = 1.0
    ...
    dill.dump(artifact, open(output_path, 'wb'))
    return artifact
...
if __name__ == '__main__':
    ...
    ndc2atc_file = './RXCUI2atc4.csv'   # NDC code to ATC-4 code mapping file，用于读取xnorm到ATC
    cid_atc = './drug-atc.csv'              # drug（CID） to ATC code mapping file，用于处理DDI表
    ndc_rxnorm_file = './ndc2RXCUI.txt'    # NDC to xnorm mapping file
    ...
    dill.dump(atc3toSMILES, open('./atc3toSMILES.pkl', "wb"))
    ...
    filter_set = ['2848', '5672', '2765', '2554', '9992', '7895', '5191', '7872', '99931', '4582', '9999', '585', '7990', '2387', '7070', '99939', '5678', '99932', 'V461', '2773', '2873', '9964', '6000', '99989', '5302', '9993']
    diag_pd = diag_pd[~diag_pd['ICD9_CODE'].isin(filter_set)]
    diag_pd = diag_pd.reset_index(drop=True)
    ...
    filter_set = ['3601', '3605', '324', '325', '323', '374', '684']
    pro_pd = pro_pd[~pro_pd['ICD9_CODE'].isin(filter_set)]
    ...
    _build_concept_cooccurrence(
        records,
        len(diag_voc.idx2word),
        len(pro_voc.idx2word),
        len(med_voc.idx2word),
        output_path='concept_cooccurrence.pkl'
    )
    print('concept cooccurrence matrix saved to concept_cooccurrence.pkl')
