...
def ndc2atc4(med_pd):
    ...
    med_pd['ndc'] = med_pd['ndc'].map(lambda x: x[:5])
    med_pd = med_pd.drop_duplicates()    
    med_pd = med_pd.reset_index(drop=True)
    # print(med_pd)
    return med_pd
...
def diag_process(diag_file):
    ...
    def filter_2000_most_diag(diag_pd):
        ...
        diag_pd = diag_pd[diag_pd['icd_code'].isin(diag_count.loc[:2000, 'icd_code'])]
...
def filter_1000_most_pro(pro_pd):
    ...
    pro_pd = pro_pd[pro_pd['icd_code'].isin(pro_count.loc[:2000, 'icd_code'])]
...
def get_ddi_matrix(records, med_voc, ddi_file):
    ...
    with open(cid_atc, 'r') as f:
        for line in f:
            ...
            atcs = line_ls[1:]
            for atc in atcs:
                if len(atc3_atc4_dic[atc[:5]]) != 0:
                    cid2atc_dic[cid].add(atc[:5])
...
if __name__ == '__main__':
    ...
    datadir = '../../mimic4/'
    med_file = datadir + "hosp/prescriptions.csv" # '/srv/local/data/physionet.org/files/mimiciii/1.4/PRESCRIPTIONS.csv'
    diag_file = datadir + "hosp/diagnoses_icd.csv"  # '/srv/local/data/physionet.org/files/mimiciii/1.4/DIAGNOSES_ICD.csv'
    procedure_file = datadir + "hosp/procedures_icd.csv" # '/srv/local/data/physionet.org/files/mimiciii/1.4/PROCEDURES_ICD.csv'
    admission_file = datadir + "core/admissions.csv"

    med_structure_file = './atc3toSMILES_4.pkl'
