...
def _prepare_pretraining_records(records):
    ...
    for patient_idx, patient in enumerate(records):
        ...
        for visit_order, visit in enumerate(patient):
            ...
            graph_med_codes = [] if visit_order == last_visit_idx else med_codes
            if visit_order == last_visit_idx:
                terminal_lookup.add((patient_idx, visit_order))
                stats['heldout_visits'] += 1
            new_visit = [diag_codes, proc_codes, graph_med_codes, med_codes]
            if len(visit) > 3:
...
def build_drug_hypergraph(records_path: str,
    ...
    for patient_idx, patient in enumerate(records):
        ...
        for visit in patient:
            ...
            label_med_codes = visit[3] if usage == 'pretrain' and len(visit) > 3 else med_codes
            ...
            label_med_nodes = [_global_id(med_offset, code) for code in label_med_codes]
