...
from pretrain.checkpoint_io import save_training_checkpoint

from pretrain.data_builder import build_drug_hypergraph
from pretrain.masking import sample_visit_masks
from pretrain.model import DrugHypergraphPretrainer, compute_initializer_output_dim
from pretrain.clustering import DEC, PredictionHead, weighted_cluster_average
from pretrain.models_ext import SetGNNExtended
...
def collect_incident_training_concepts(visit_summaries,
                                       edge_index: torch.Tensor) -> Tuple[Set[int], Set[int]]:
    """Select train concepts that can receive a representation from the hypergraph."""
    candidate_ids: Set[int] = set()
    for visit in visit_summaries:
        candidate_ids.update(visit.diag_nodes)
        candidate_ids.update(visit.proc_nodes)
        candidate_ids.update(visit.med_nodes)

    if edge_index.numel() == 0:
        return set(), candidate_ids

    incident_ids = set(torch.unique(edge_index[0]).detach().cpu().tolist())
    return candidate_ids & incident_ids, candidate_ids - incident_ids
...
def main():
    ...
    parser.add_argument('--warmup1-epochs', type=int, default=50,
    ...
    train_concept_ids, excluded_train_concept_ids = collect_incident_training_concepts(
    ...
    if excluded_train_concept_ids:
        logger.warning(
            'Excluded %d non-incident train concepts from DEC clustering; '
            'they remain medication targets (first IDs: %s)',
            len(excluded_train_concept_ids),
            sorted(excluded_train_concept_ids)[:20],
        )
    ...
    logger.info('Terminal supervision targets prepared: train=%d', int(terminal_indices.numel()))
    ...
    full_best_visit_centroids: Optional[torch.Tensor] = None
    full_best_node_centroids: Optional[torch.Tensor] = None
    full_best_epoch = 0
        ...
        if has_eval_split and warmup2_patience > 0:
            ...
            if eval_ja is not None and eval_ja > warmup2_best_eval + 1e-6:
                ...
                save_training_checkpoint(
                    {'model': warmup2_best_state, 'epoch': epoch, 'eval_ja': eval_ja},
                    warmup2_best_path,
                )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if train_concept_index_tensor.numel() > 0:
            max_concept_index = int(train_concept_index_tensor[-1].item())
            if max_concept_index >= concept_emb.size(0):
                raise RuntimeError(
                    'DEC concept selection is inconsistent with the encoder output: '
                    f'max selected index {max_concept_index}, available rows {concept_emb.size(0)}. '
                    'Check that clustering concepts are incident to the hypergraph.'
                )
        ...
        if has_eval_split and full_patience > 0:
            ...
            if eval_ja is not None and eval_ja > full_best_eval + 1e-6:
                ...
                full_best_visit_centroids = visit_cluster.mean.detach().cpu().clone()
                full_best_node_centroids = node_cluster.mean.detach().cpu().clone()
                save_training_checkpoint({
                    'model': full_best_state,
                    'epoch': epoch,
                    'eval_ja': eval_ja,
                    'visit_centroids': full_best_visit_centroids,
                    'node_centroids': full_best_node_centroids,
                }, full_best_path)
    ...
    if args.save_checkpoint:
        ...
        payload = {
            ...
            'visit_centroids': full_best_visit_centroids if full_best_visit_centroids is not None else visit_cluster.mean.detach().cpu(),
            'node_centroids': full_best_node_centroids if full_best_node_centroids is not None else node_cluster.mean.detach().cpu(),
        }
        save_training_checkpoint(payload, ckpt)
