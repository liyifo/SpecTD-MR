...
if TYPE_CHECKING:
    from pretrain.data_builder import VisitSummary
...
def _split_patient_indices(num_patients: int):
    ordered = list(range(num_patients))
    train_end = int(len(ordered) * 2 / 3)
    remaining = len(ordered) - train_end
    test_len = remaining // 2
    train_idx = ordered[:train_end]
    test_idx = ordered[train_end:train_end + test_len]
    eval_idx = ordered[train_end + test_len:]
    ...
    def __init__(self,
        ...
        for patient_idx in selected:
            ...
            for idx, visit in enumerate(visits):
                ...
                raw_visit = raw_records[patient_idx][visit.order_index]
                raw_visit_time = (
                    raw_visit[3] if len(raw_visit) > 3 else visit.order_index
                )
                try:
                    visit_time = float(raw_visit_time)
                except (TypeError, ValueError):
                    visit_time = float(visit.order_index)
                if not math.isfinite(visit_time):
                    visit_time = float(visit.order_index)
            ...
            history_medications = torch.zeros(
                len(sequence_visits), med_vocab_size, dtype=torch.float32,
            )
            for visit_idx, visit in enumerate(sequence_visits[:-1]):
                history_codes = [node - med_offset for node in visit.med_nodes]
                history_medications[visit_idx, history_codes] = 1.0
...
def collate_sequence_batch(batch: List[SequenceExample]):
    ...
    visit_times = torch.zeros(batch_size, max_visits, dtype=torch.float32)
    ...
    med_vocab_size = int(targets.size(1)) if batch else 0
    history_medications = torch.zeros(batch_size, max_visits, med_vocab_size, dtype=torch.float32)
    ...
    for b_idx, example in enumerate(batch):
        ...
        if v_len > 0:
            ...
            times = torch.tensor(
                [visit.visit_time for visit in example.visits], dtype=torch.float32,
            )
            visit_orders[b_idx, :v_len] = orders
            visit_times[b_idx, :v_len] = times
            visit_node_masks[b_idx, :v_len, :n_len] = example.visit_node_masks[:v_len, :n_len]
            history_medications[b_idx, :v_len] = example.history_medications[:v_len]
